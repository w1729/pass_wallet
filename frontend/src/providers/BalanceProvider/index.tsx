...
function useBalanceHook() {
  ...
  const getBalanceUSD = useCallback(async (address: Hex) => {
    ...
    const priceData = await fetch("/api/price?ids=injective-protocol&currencies=usd");
    const json = await priceData.json();
    const price: number = Math.trunc(json["injective-protocol"].usd * 100);
