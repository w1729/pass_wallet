...
export async function GET(_req: Request, { params }: { params: { id: Hex } }) {
  ...
  const user = await PUBLIC_CLIENT.readContract({
    address: "0x2279B7A0a67DB372996a5FaB50D91eAA73d2eBe6" as Hex,
    abi: FACTORY_ABI,
  ...
  if (user?.account) {
    const result = await fetch(
      `https://api.socialscan.io/monad-testnet/v1/developer/api?module=account&action=balance&address=${user.account}&tag=latest&apikey=xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx`,
      { cache: "no-store" },
    );
