...
export default function SendTxModal() {
  ...
  const submitTx = async (e: any) => {
    ...
    try {
      const price: { "injective-protocol": { usd: number } } = await (
        await fetch("/api/price?ids=injective-protocol&currencies=usd")
      ).json();
      ...
      const ethClient = createPublicClient({
        transport: http("http://127.0.0.1:8545"), // Use your Ethereum node port, not the bundler
      });
        ...
        calls: [
          {
            ...
            value:
              BigInt(parseEther(userInputAmount)) /
              (BigInt(Math.trunc(price["injective-protocol"].usd * 100)) / BigInt(100)), // 100 is the price precision
            data: emptyHex,
          },
        ],
        maxFeePerGas: maxFeePerGas as bigint,
        ...
        <Flex direction="column" justify="center" align="center" grow="1" gap="5">
          {true ? (
            <>
              ...
              <Link
                href={`https://sepolia.basescan.org/tx/${txReceipt?.receipt?.transactionHash}`}
                target="_blank"
  ...
  return (
    <Flex direction="column" style={{ flexGrow: 1, width: "100%" }} gap="5">
      {!txReceipt && !isLoading && (
        <Heading as="h2" size={"8"} style={{ color: "rgba(0, 0, 0, 0.95)" }}>
          Send transaction
        </Heading>
      )}
      {!txReceipt && !isLoading && (
            ...
            <Flex direction="column">
              ...
              >
                <Flex direction="column" gap="1">
                  ...
                  <TextField.Root>
                    <TextField.Slot style={{ color: "rgba(0, 0, 0, 0.95)", paddingLeft: "1rem" }}>
                      To:
                    </TextField.Slot>
                    <TextField.Input
                      ref={addressInputRef}
                    ...
                    />
                    <TextField.Slot style={{ color: "rgba(0, 0, 0, 0.95)" }}>
                      {!destination && ensIsLoading && (
                        <Spinner style={{ margin: 0, width: 20, height: 20 }} />
                      )}
                      {destination && (
                        <CheckCircledIcon height="20" width="20" color="rgba(0, 0, 0, 0.95)" />
                      )}
                      {!destination && !ensIsLoading && (
                        <CrossCircledIcon
                          height="20"
